...
class Convolution2D:
    def __init__(self, Input_planes, Out_planes, Conv_H, Conv_W, Stride_H, Stride_W, Padding, Relu, Weight,
                 Bias=np.any([False]), dila_rate=1, Precision=10000):
    ...
    def forward(self, data, dma, Load_Input=1, Load_Weight=1):
        ...
        Configuration = xlnk.cma_array(shape=(18,), dtype=np.int32)
        Configuration[0] = 0
        ...
        Configuration[17] = self.dila_rate
...
class FC:
    ...
    def forward(self, data, dma, Load_Input=1):
        ...
        in_buffer = xlnk.cma_array(shape=(3000,), dtype=np.int32)   # set dma buffer size
        data = np.multiply(data, self.Precision)    
        x = data.flatten()  # to 1D 
        size = data.shape   # D X
...
class Batch_Norm:
    def __init__(self, Input_Size, Weight, pRelu, Precision=10000):
        self.Input_planes = Input_Size
        assert len(Weight.shape) == 1, 'Parameters of BN should be reshaped to (P)'
        self.Weight = np.multiply(Weight, Precision)
    ...
    def forward(self, data, dma, Load_Input=1, Load_Weight=1):
        ...
        Configuration = xlnk.cma_array(shape=(18,0), dtype=np.int32)
        Configuration[0] = 3
        Configuration[1] = x.shape[0]
        Configuration[2] = size[0]
        Configuration[3] = size[1]
        Configuration[4] = size[2]
        Configuration[5] = Load_Input
        Configuration[6] = Load_Weight
        Configuration[7] = self.Precision
        Configuration[8] = self.pRelu
        Configuration[9] = 0
        Configuration[10] = 0
        Configuration[11] = 0
        Configuration[12] = 0   
        ...
        # upload Paras inside FPGA
        # gamma, beta, mean, variance , pRelu sequentially (total len = size[0]*5)
        ...
        # Return result to float format and reshape
        Output = np.divide(Output, self.Precision).reshape(self.Input_planes, Out_H, Out_W)
        return Output
